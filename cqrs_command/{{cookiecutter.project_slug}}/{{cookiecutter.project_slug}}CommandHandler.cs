...
public sealed class {{cookiecutter.project_slug}}CommandHandler()
    : ICommandHandler<{{cookiecutter.project_slug}}Command, {{cookiecutter.project_slug}}Response>
{
    public async Task<Result<{{cookiecutter.project_slug}}Response>> Handle({{cookiecutter.project_slug}}Command command, CancellationToken cancellationToken)
    {
